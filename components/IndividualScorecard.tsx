...
interface AIDeepDiveMonitor {
  ...
  hate_speech_justification?: string[];
}
...
export default function IndividualScorecard({
  ...
  const [hateData, setHateData] = useState<LiveHateData | null>(null);
  const [hateLoading, setHateLoading] = useState<boolean>(false);
  ...
  // Fetch live constitutional evaluation matrix
  useEffect(() => {
    let isMounted = true;
  ...
  const aiMonitor: AIDeepDiveMonitor = leader.ai_monitor_data || {
    ...
    hate_speech_justification:
      (leader.hate_speech_score ?? 0) > 0
        ? [
            `Automated linguistic analysis flagged an elevated polarization score of ${leader.hate_speech_score}%.`,
          ]
        : [],
  };
  ...
  const handlePlayAudio = async () => {
    if (isPlaying) return; // Prevent overlapping requests
    ...
    text += `Hate speech and divisive rhetoric index is at ${leader?.hate_speech_score ?? "0"} percent. `;
    if (
      aiMonitor.hate_speech_justification &&
      aiMonitor.hate_speech_justification.length > 0
    ) {
      text += `Flagged speech instances include: ${aiMonitor.hate_speech_justification.join(" ")} `;
    }
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/v1/accessibility/tts`, {
        ...
        body: JSON.stringify({ text_content: sanitizedText, voice: "onyx" }),
      });
      ...
      // Free up button when playback finishes normally or crashes
        ...
        <div className="flex flex-col gap-6 bg-slate-950 p-5 sm:p-6 rounded-xl border border-slate-800/60 shadow-inner">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            ...
            <button
              ...
              disabled={isPlaying}
              className={`ml-4 rounded-full p-2 transition flex items-center gap-2 ${
                isPlaying
            ...
            <div className="w-full lg:w-auto flex items-center gap-2 bg-slate-900 p-2 rounded-xl border border-slate-800 overflow-x-auto">
              ...
              <button
                onClick={() => setActiveMetricTab("hate")}
                className={`flex-1 min-w-[90px] p-2.5 rounded-lg transition-all duration-200 text-center ${
                  activeMetricTab === "hate"
                    ? "bg-slate-950 border border-slate-700 shadow-md scale-105"
                    : "hover:bg-slate-800/50 border border-transparent"
                }`}
              >
        ...
        <div className="grid gap-6 lg:grid-cols-2 items-start">
          ...
          <div className="rounded-xl border border-slate-800/60 bg-slate-950 p-5 sm:p-6 flex flex-col h-full min-h-[380px]">
            ...
            <div className="flex-1 space-y-4">
              ...
              {activeMetricTab === "hate" && (
                <div className="animate-fadeIn space-y-3">
                  <span className="font-bold text-purple-400 text-base sm:text-lg block">
                    📢 Hate Speech & Polarization Index
                  </span>
                  <p className="text-sm sm:text-base text-slate-400 leading-relaxed italic">
                  ...
                  <div className="space-y-3 pt-2">
                    {hateLoading ? (
                      <div className="py-8 text-center bg-slate-900/40 rounded-xl border border-slate-800 shadow-inner">
                        <div className="inline-block h-7 w-7 animate-spin rounded-full border-2 border-purple-500 border-t-transparent"></div>
                        <p className="text-sm text-slate-400 mt-3 font-medium animate-pulse">
                          Scanning digital broadcasts and recent media
                          transcripts...
                        </p>
                      </div>
                    ) : hateData && hateData.statements.length > 0 ? (
                      hateData.statements.map((stmt, idx) => (
                        <div
                          key={idx}
                          className="flex gap-3 items-start text-base sm:text-lg text-slate-200 bg-slate-900/50 p-4 rounded-xl border border-slate-800"
                        >
                          <span className="text-purple-400 shrink-0 mt-0.5 text-xl">
                            ☣️
                          </span>
                          <div className="space-y-2 w-full">
                            ...
                            <div className="flex justify-between items-center mt-3 border-t border-slate-800/60 pt-2.5">
                              <span className="text-xs uppercase font-bold text-slate-500 tracking-wider">
                                Severity Metric
                              </span>
                              <span className="text-xs font-mono font-bold text-purple-300 bg-purple-950/50 border border-purple-900/50 px-2.5 py-1 rounded-md">
                                {stmt.severity_score}%
                              </span>
                            </div>
                          </div>
                      ...
                      <div className="p-4 rounded-xl bg-slate-900/40 border border-slate-800 text-sm text-slate-400 italic">
                        {hateData?.verdict ||
                          "No significant hate speech or inciting rhetoric flagged in recent public recordings."}
                      </div>
                    )}
                  </div>
                </div>
